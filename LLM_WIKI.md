...
| `src/lib/email-template.ts` | `brandedEmailHtml()` + `escapeHtml()` — brand-parameterized HTML email wrapper; `showTitle:false` skips the `<h1>` when the subject is already restated in the body, `bodyHtml` takes pre-rendered HTML instead of auto-paragraphed `bodyText`; `paragraphHtml(p, {rtl})` (single styled `<p>`; `rtl` for right-to-left scripts) and `renderParagraphs(text, {rtl})` (blank-line split) and `coverLinkBlock()` (optional cover image + accent CTA button + plain-URL fallback, for "read the full thing online" emails like newsletters/reports) are exported so callers can splice custom blocks into the body and pass the result as `bodyHtml`. `LINK_MARKER` (`{{link}}`) + `renderParagraphsWithLinkBlock(text, blockHtml, {rtl})` render a body with each marker line replaced by the block (no marker → appended after the copy) — used by PIF's newsletter email and its audience composer's cover-image sends; `stripLinkMarker(text)` removes marker lines (e.g. before handing copy to a translator) | — |
| `src/lib/translation.ts` | Bilingual email support, **SDK-free** so the client composer can import the language list: `TRANSLATION_LANGUAGES` (superset registry: Turkish, te reo Māori, Arabic (`rtl`), Spanish — each app's composer offers its subset via `translationLanguagesFor(codes)`; PIF tr/mi/ar, Raindrops tr/es), `findTranslationLanguage(code)`, `TRANSLATION_SCHEMA` + `translationSystem({lang, orgName})` (the prompt routes pair with `draftStructured`), and `bilingualBodyHtml({englishText, translation, lang})` (English original → divider → translation). Paired with `translateBody()` in `ai.ts` | — |
| `src/lib/email.ts` | `sendEmail()` — Resend transport; caller supplies the brand `from`. `sendEmailBatch()` — chunked `/emails/batch` sender (100/request, 2 req/s pacing) with **`x-batch-validation: permissive`**: strict mode (Resend's default) rejects a whole 100-email request over one malformed address, silently skipping 99 good recipients — permissive queues the good ones and returns the bad ones per-address in `failures[{to, reason}]`. Both log-only when `RESEND_API_KEY` unset | env (`RESEND_*`) |
| `src/lib/ai.ts` | `draftStructured(args, requiredKeys)` — one structured Claude call (`claude-opus-4-8`, adaptive thinking, `output_config.format` JSON schema) returning a tagged `{ok}` outcome; `aiConfigured()` guard; `guidanceLine()`; `translateBody({englishText, lang, orgName})` wraps `draftStructured` with the `translation.ts` prompt/schema for bilingual sends. Each admin `/generate` route = a system prompt + schema | `@anthropic-ai/sdk` (peer, **optional** — dormant until an app imports it) |
