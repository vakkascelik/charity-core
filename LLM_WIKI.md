...
| `src/lib/translation.ts` | Bilingual email support, **SDK-free** so the client composer can import the language list: `TRANSLATION_LANGUAGES` (Turkish, te reo Māori, Arabic — add here to widen the dropdown; `rtl` flag), `findTranslationLanguage(code)`, `TRANSLATION_SCHEMA` + `translationSystem({lang, orgName})` (the prompt routes pair with `draftStructured`), and `bilingualBodyHtml({englishText, translation, lang})` (English original → divider → translation). Paired with `translateBody()` in `ai.ts` | — |
